...
public class AdminUserController {
     ...
     * - Retorna todos los usuarios de la organización sin importar su estado
     * 
     ...
     */
    @GetMapping
    @Operation(summary = "Listar usuarios de la organización con roles", description = "Retorna lista paginada de todos los usuarios que pertenecen a la organización del token JWT con sus roles asignados. "
            +
            "Soporta búsqueda por email/nombre. " +
    ...
    public ResponseEntity<ListUsersResponseDto> listUsers(
            ...
            @Parameter(description = "Número de página (base 1)", example = "1") @RequestParam(defaultValue = "1") Integer page,

            @Parameter(description = "Elementos por página (máximo 100)", example = "20") @RequestParam(defaultValue = "20") Integer limit,

            @Parameter(description = "Búsqueda en email o nombre (case-insensitive)", example = "juan") @RequestParam(required = false) String busqueda) {

        log.info("Solicitud de listado de usuarios por admin: {} (org: {}, page={}, limit={}, busqueda={})",
                tokenValidation.usuarioId(), tokenValidation.organizacionId(),
                page, limit, busqueda);
        ...
        // 3. Delegar al servicio con parámetros opcionales (sin filtro de estado)
