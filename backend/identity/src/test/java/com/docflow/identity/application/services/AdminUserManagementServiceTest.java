...
class AdminUserManagementServiceTest {
    ...
    /**
     * Helper method para crear mocks de UserWithRolesProjection (interface-based
     * projection).
     */
    private UserWithRolesProjection createProjection(
            Long usuarioId, String email, String nombreCompleto, String estado,
            Long rolId, String rolCodigo, String rolNombre, OffsetDateTime fechaCreacion) {
        UserWithRolesProjection mock = mock(UserWithRolesProjection.class);
        when(mock.getUsuarioId()).thenReturn(usuarioId);
        when(mock.getEmail()).thenReturn(email);
        when(mock.getNombreCompleto()).thenReturn(nombreCompleto);
        when(mock.getEstado()).thenReturn(estado);
        when(mock.getRolId()).thenReturn(rolId);
        when(mock.getRolCodigo()).thenReturn(rolCodigo);
        when(mock.getRolNombre()).thenReturn(rolNombre);
        when(mock.getFechaCreacion()).thenReturn(fechaCreacion);
        return mock;
    }
    ...
    void testListUsersSuccess() {
        ...
        var proyeccion1Usuario1 = createProjection(
                101L, "admin@test.com", "Admin User", "ACTIVO",
                1L, "ADMIN", "Administrador", NOW);
        var proyeccion2Usuario1 = createProjection(
                101L, "admin@test.com", "Admin User", "ACTIVO",
                3L, "OPERATOR", "Operador", NOW);
        var proyeccion1Usuario2 = createProjection(
    ...
    void testLimitForcedTo100() {
        // Arrange
        var proyeccion = createProjection(
